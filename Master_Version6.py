...
def predictNext(Team1, Team2):
    ...
    if (int(Liste[0]) + int(Liste[2]) + int(Liste[1]) < 10):
        ...
        return str(Liste[2]) + "% " + "Auswärtssieg: "
        + str(Liste[0]) + " - " + str(Liste[1]) + " - " + str(Liste[2])

    elif Liste[1] >= Liste[0] and Liste[1] >= Liste[2]:
        return str(Liste[1]) + "% " + "Unentschieden: "
        + str(Liste[0]) + " - " + str(Liste[1]) + " - " + str(
            Liste[2])
...
class GUI:
    ...
    def erstelleKommenderSpieltag(self):
        ...
        labelNamesHeim = [self.heim1, self.heim2,
                          ...
                          self.heim5, self.heim6, self.heim7,
                          self.heim8, self.heim9]
        labelNamesGast = [self.gast1, self.gast2, self.gast3,
                          self.gast4, self.gast5,
        ...
        for i, (a, b, c) in enumerate(zip(labelNamesHeim, labelNamesGast, labelNamesErgebnis)):
            ...
            c = tk.Label(self.Window, text=predictNext(kommendeMannschaften[2 * i], kommendeMannschaften[2 * i + 1]),
                         width=55).grid(column=2, row=spieltag + i + 1)
    ...
    def starteMachineLearningTraining(self):
        ...
        if VonSaison.isdigit() and BisSaison.isdigit() and VonTag.isdigit() and BisTag.isdigit():
            #Todo: Binde Funktion ein die gewünschten Daten ausDatenbank holt
            Schranken = Schnittstelle(int(
                VonSaison), int(VonTag), int(BisSaison), int(BisTag)
                )
            if Schranken[0] < 0:
                self.ErrorLabel.config(
                    text="Saison zwischen 2009 und 2018, Tag zwischen 1 und 34"
                    )
            else:
    ...
    def predict(self):
        if self.MV.get() == 1:
            if self.var1.get() == self.var2.get():
                # Dasselbe Team soll nicht gegen sich selbst spielen können
                self.labelGewinnHeimNum.config(
                    text="Error: dasselbe Team ausgewählt"
                    )
                self.labelUnentschiedenNum.config(text="")
                self.labelVerlustGastNum.config(text="")
            else:
                Liste = Gewinnwahrscheinlichkeit(
